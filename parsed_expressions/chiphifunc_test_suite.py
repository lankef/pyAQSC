...
chi = np.linspace(-np.pi, np.pi, n_grid_chi)
phi = points

# Evaluate a callable on 'points' (defined above)
def evaluate(func):
    return(func(chi, phi.reshape(-1,1)))
...
def display(array, complex=True):
    plt.pcolormesh(chi, phi, np.real(array))
    plt.colorbar()
    plt.show()
    if complex:
        plt.pcolormesh(chi, phi, np.imag(array))
        plt.colorbar()
        plt.show()
...
def compare_chiphifunc(guess, ans):
    print('Guess')
    ax1 = plt.subplot(121)
    ax1.plot(np.real(guess.content).T, title='Real')
    ax2 = plt.subplot(122)
    ax2.plot(np.imag(guess.content).T, title='Imaginary')
    plt.show()
    print('Answer')
    ax1 = plt.subplot(121)
    ax1.plot(np.real(ans.content).T, title='Real')
    ax2 = plt.subplot(122)
    ax2.plot(np.imag(ans.content).T, title='Imaginary')
    plt.show()

    print('Error')
    ax1 = plt.subplot(121)
    ax1.plot(np.real((ans-guess).content).T, title='Real')
    ax2 = plt.subplot(122)
    ax2.plot(np.imag((ans-guess).content).T, title='Imaginary')
    plt.show()
    (ans-guess).display()
