...
def eval_D3_RHS_m_LHS(n, X_coef_cp, Y_coef_cp, Z_coef_cp, B_theta_coef_cp, 
    ...
    def sum_arg_15(i268):
        ...
        return(X_coef_cp[i268]*diff(Y_coef_cp[n-i268],'chi',1))
    
    def sum_arg_14(i264):
        # Child args for sum_arg_14
        return(Y_coef_cp[i264]*diff(X_coef_cp[n-i264],'chi',1))
    
    def sum_arg_13(i270):
        # Child args for sum_arg_13
        return(X_coef_cp[i270]*diff(Z_coef_cp[n-i270],'chi',1))
    
    def sum_arg_12(i266):
        # Child args for sum_arg_12
        return(Z_coef_cp[i266]*diff(X_coef_cp[n-i266],'chi',1))
    
    def sum_arg_11(i261):
        # Child args for sum_arg_11    
        def sum_arg_10(i262):
            # Child args for sum_arg_10
            return(diff(Z_coef_cp[i262],'chi',1)*diff(Z_coef_cp[(-n)-i262+2*i261],'chi',1)*is_seq(n-i261,i261-i262))
        
        return(is_seq(0,n-i261)*iota_coef[n-i261]*is_integer(n-i261)*py_sum(sum_arg_10,0,i261))
    
    def sum_arg_9(i260):
        # Child args for sum_arg_9
        return(diff(Z_coef_cp[i260],'chi',1)*diff(Z_coef_cp[n-i260],'phi',1))
    
    def sum_arg_8(i257):
        # Child args for sum_arg_8    
        def sum_arg_7(i258):
            # Child args for sum_arg_7
            return(diff(Y_coef_cp[i258],'chi',1)*diff(Y_coef_cp[(-n)-i258+2*i257],'chi',1)*is_seq(n-i257,i257-i258))
        
        return(is_seq(0,n-i257)*iota_coef[n-i257]*is_integer(n-i257)*py_sum(sum_arg_7,0,i257))
    
    def sum_arg_6(i256):
        # Child args for sum_arg_6
        return(diff(Y_coef_cp[i256],'chi',1)*diff(Y_coef_cp[n-i256],'phi',1))
    
    def sum_arg_5(i253):
        # Child args for sum_arg_5    
        def sum_arg_4(i254):
            # Child args for sum_arg_4
            return(diff(X_coef_cp[i254],'chi',1)*diff(X_coef_cp[(-n)-i254+2*i253],'chi',1)*is_seq(n-i253,i253-i254))
        
        return(is_seq(0,n-i253)*iota_coef[n-i253]*is_integer(n-i253)*py_sum(sum_arg_4,0,i253))
    
    def sum_arg_3(i252):
        # Child args for sum_arg_3
        return(diff(X_coef_cp[i252],'chi',1)*diff(X_coef_cp[n-i252],'phi',1))
