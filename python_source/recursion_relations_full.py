...
class Equilibrium:
    ...
    def load(filename):
        ...
        constant['kap_p']\
            = ChiPhiFuncGrid(raw_constant['kap_p'])
        constant['dl_p']\
            = raw_constant['dl_p']
        constant['tau_p']\
            = ChiPhiFuncGrid(raw_constant['tau_p'])
        constant['eta']\
            = raw_constant['eta']
    ...
    def iterate_B_psi_nm30_Y_nm10_p(self, n_eval, \
        X_coef_cp, Y_coef_cp, Z_coef_cp, \
        B_theta_coef_cp, B_psi_coef_cp, \
        B_alpha_coef, B_denom_coef_c, \
    ...
    def iterate_B_theta_n0_cp(self, loop_suppressing_B_theta_nm1, B_theta_nm1):
        constant_component = (loop_suppressing_B_theta_nm1 - \
        - self.coeff_B_theta_nm1*B_theta_nm1 \
        - self.coeff_dc_B_theta_nm1*B_theta_nm1.dchi() \
        - self.coeff_dp_B_theta_nm1*B_theta_nm1.dphi()).get_constant().filter()
        B_theta_n0,_ = solve_integration_factor(
            coeff = self.coeff_B_theta_n0.content,
            coeff_dp = self.coeff_dp_B_theta_n0.content,
            f = constant_component.content, integral_mode = 'spline')
        return(ChiPhiFuncGrid(B_theta_n0))
    ...
    def prepare_constants(self):
        ...
        ###################
        # The looped equation is different from what I thought.
        # Commenting these out for now
        # If these are reused later, mind that the coeff_pre's
        # MIGHT BE FLOATS.
        ###################
