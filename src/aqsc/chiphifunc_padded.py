...
class ChiPhiEpsFuncPadded:
    ...
    ''' II.3 List-like operations, mirroring ChiPhiEpsFunc '''
    ...
    def _map_orders(self, fn):
        ...
        for n in range(self.n_max + 1):
            ...
            if code != 0:
                ...
                # fn operates at the container's shared chi_cap width, then
                # gets trimmed back down to this order's true natural width
                # so the triangular store stays minimal.
    ...
    ''' II.5 Arithmetic (operating on the whole series, mirroring ChiPhiEpsFunc) '''
...
def _classify_item(item, n: int, len_phi: int):
    ...
    where content is always a real (n+1, len_phi) array (zeros for special/
    error items -- specialness is tracked via special_code, not via the
    stored array, so it never contaminates arithmetic with NaN).
    '''
    ...
    if jnp.ndim(item) == 0:
        ...
        return 0, content
    raise TypeError(f'ChiPhiEpsFuncPadded: item at order {n} is neither ChiPhiFuncPadded, '
                     f'ChiPhiFunc, nor a scalar: {type(item)}.')


tree_util.register_pytree_node(
    ChiPhiEpsFuncPadded, ChiPhiEpsFuncPadded._tree_flatten, ChiPhiEpsFuncPadded._tree_unflatten
)
