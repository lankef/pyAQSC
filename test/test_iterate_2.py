...
class TestCircularAxis(unittest.TestCase):
    ...
    def test_governing(self):
        ...
        self.assertTrue(I.filter(20).get_max()<n_2_tolerance)
        print('II residue:')
        aqsc.print_fractional_error(II.filter(20).content, 0)
        self.assertTrue(II.filter(20).get_max()<n_2_tolerance)
        print('III residue:')
        aqsc.print_fractional_error(III.filter(20).content, 0)
        self.assertTrue(III.filter(20).get_max()<n_2_tolerance)
        ...
        print('I residue:')
        aqsc.print_fractional_error(I.filter(20).content, 0)
        self.assertTrue(I.filter(20).get_max()<n_4_tolerance)
        print('II residue:')
        aqsc.print_fractional_error(II.filter(20).content, 0)
        self.assertTrue(II.filter(20).get_max()<n_4_tolerance)
        print('III residue:')
        aqsc.print_fractional_error(III.filter(20).content, 0)
        self.assertTrue(III.filter(20).get_max()<n_4_tolerance)
